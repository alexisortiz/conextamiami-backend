 ...
 */
export async function loadBridgeTokenFromSsm(): Promise<void> {
  const paramName = process.env.BRIDGE_SERVER_TOKEN_SSM_PARAM?.trim();
  if (!paramName) return;

  if (process.env.BRIDGE_SERVER_TOKEN?.trim()) return;
  ...
  try {
    ...
    console.error(
      `Could not load BRIDGE_SERVER_TOKEN from SSM (${paramName}). ` +
        'Create it with ./scripts/put-bridge-token-ssm.sh or set BRIDGE_SERVER_TOKEN.',
      err,
    );
  }
}
